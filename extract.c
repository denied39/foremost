 ...
 *Return: A pointer to where the EOF of the RIFF is in the current buffer
 **********************************************************************************/
unsigned char *extract_vp(f_state *s, u_int64_t c_offset, unsigned char *foundat, u_int64_t buflen,
							s_spec *needle, u_int64_t f_offset, char *type)
{
	unsigned char	*buf = foundat;
	unsigned char	*extractbuf = NULL;
	...
	if (strncmp((char *) &foundat[0], "STEVE4769", 9) == 0) /*Sanity Check*/
		{
		if (size > 0 && size <= needle->max_len && size <= buflen)
		{
#ifdef DEBUG
			printf("\n	Found a VP at:=%lld,File size:=%d\n", c_offset, size);
#endif
							...
							s_spec *needle, u_int64_t f_offset)
{
  ...
  else if (needle->type == VP)
    {
    needle->suffix = "vp";
    return extract_vp(s, c_offset, foundat, buflen, needle, f_offset, "vp");
    }
